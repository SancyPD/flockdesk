...
class RecentTicketsScreen extends StatefulWidget {
  ...
  final Function(int) onAllViewExpansionToggle; // Callback for toggling view expansion
  final Function(Widget, String) onMainScreenChange; // Callback to change main screen

  const RecentTicketsScreen({
    super.key,
...
class _RecentTicketsScreenState extends State<RecentTicketsScreen> {
  ...
  void initState() {
    ...
    _loadRecentTickets(refresh: true);
    _preloadTrashCount();
  }
  ...
  Future<void> _loadRecentTickets({bool refresh = false}) async {
    try {
      if (refresh) {
        setState(() {
          _isLoading = true;
      ...
      final ticketResponse = await _ticketService.getRecentTickets(
        dateSort: 2,
        idSort: 0,
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: Builder(
            builder: (context) =>
                IconButton(
                  icon: Image.asset(
                      'assets/images/menu.png', width: 24, height: 24),
          ...
          title: const Text(
            'Recent Tickets',
            style: TextStyle(
                color: Colors.black,fontFamily: 'Inter', fontWeight: FontWeight.w500, fontSize: 18),
          ),
          actions: [
            IconButton(
              icon: Image.asset(
              ...
              onPressed: () async {
                ...
                // If ticket was created successfully, refresh the recent tickets list
                if (result == true) {
                  _loadRecentTickets(refresh: true);
                }
              },
            ...
            IconButton(
              icon: Image.asset('assets/images/notification_ic.png',
                  width: 24, height: 24),
              onPressed: () {
        ...
        body: _isLoading
            ...
            : RefreshIndicator(
          onRefresh: () => _loadRecentTickets(refresh: true),
  ...
  Widget _ticketItem(BuildContext context, {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => TicketDetailsScreen(
              ...
              fromTrash: false,
              onTicketTrashed: () {
                // Refresh the recent tickets list when ticket is moved to trash
                _loadRecentTickets(refresh: true);
              },
            ),
          ),
